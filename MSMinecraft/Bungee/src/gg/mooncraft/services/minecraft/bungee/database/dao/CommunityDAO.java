...
public final class CommunityDAO {
    ...
    Methods
     */
    public static @NotNull CompletableFuture<CommunityUser> select(@NotNull ProxiedPlayer proxiedPlayer) {
        ...
        return MSMinecraftMain.getInstance().getDatabase().getDatabaseManager().executeQuery(query, resultSetIterator -> {
            if (!resultSetIterator.hasNext()) {
                return insert(new CommunityUser(proxiedPlayer.getUniqueId(), proxiedPlayer.getName(), null, null, Timestamp.from(Instant.now()), Timestamp.from(Instant.now()), proxiedPlayer.getServer().getInfo().getName())).join();
            }
            ...
            CommunityUser communityUser = new CommunityUser(proxiedPlayer.getUniqueId(), proxiedPlayer.getName(), discordId, walletId, firstJoin, Timestamp.from(Instant.now()), lastServer);
    ...
    public static @NotNull CompletableFuture<CommunityUser> update(@NotNull CommunityUser communityUser) {
        Query query = Query.single("UPDATE " + TABLE + " SET username = ?, last_join = ?, last_server = ? WHERE unique_id = ?;")
                .with(communityUser.getUsername())
                .with(communityUser.getLastJoin())
                .with(communityUser.getLastServer())
                .with(communityUser.getUniqueId().toString())
