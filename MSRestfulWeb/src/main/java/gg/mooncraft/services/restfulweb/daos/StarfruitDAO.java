...
public final class StarfruitDAO {
    ...
    private static @NotNull CompletableFuture<Map<String, Integer>> loadSkillsData(@NotNull UUID uniqueId) {
        Query query = Query.single("SELECT adventuring.user_unique_id, adventuring.level, archaeology.level, construction.level, dueling.level, farming.level, fishing.level, foraging.level, mining.level, smithing.level, warfare.level, zoology.level FROM starfruit.sf_skills_adventuring adventuring JOIN starfruit.sf_skills_archaeology archaeology USING (user_unique_id) JOIN starfruit.sf_skills_construction construction USING (user_unique_id) JOIN starfruit.sf_skills_dueling dueling USING (user_unique_id) JOIN starfruit.sf_skills_farming farming USING (user_unique_id) JOIN starfruit.sf_skills_fishing fishing USING (user_unique_id) JOIN starfruit.sf_skills_foraging foraging USING (user_unique_id) JOIN starfruit.sf_skills_mining mining USING (user_unique_id) JOIN starfruit.sf_skills_smithing smithing USING (user_unique_id) JOIN starfruit.sf_skills_warfare warfare USING (user_unique_id) JOIN starfruit.sf_skills_zoology zoology USING (user_unique_id) WHERE user_unique_id = ?;")
                .with(uniqueId)
                .build();
